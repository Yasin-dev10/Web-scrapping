...
class ScraperCrimeOnlyGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Universal Crime Only Scraper")
    ...
    def _header(self):
        ...
        tk.Label(hdr, text="Universal Crime Only Scraper",
                 font=(FONT_FAMILY, 15, "bold"),
                 bg=SURFACE, fg=TEXT).pack(side="left", pady=10)
        tk.Label(hdr, text="  —  Dhamaan xogaha crime-ka kaliya meel ka baadh",
            ...
            try:
                ...
                if len(full_text) >= 100:
                    ...
                    if cat == "crime-related":
                        ...
                        self._log_msg(f"✅ [{len(self._items)}/{target}] {title_text[:50]}", "crime")
                        self._update_stats()
        ...
        while len(self._items) < target and no_new < 10 and not self._stop_flag:
            ...
            for block in blocks:
                ...
                if cat == "crime-related":
                    ...
                    gained += 1
                    self._log_msg(f"[{len(self._items)}/{target}] {platform}: {text[:60]}...", "crime")
                    self._update_stats()
                    self._set_progress(len(self._items), target)

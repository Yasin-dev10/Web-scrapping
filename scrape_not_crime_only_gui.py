...
class ScraperNotCrimeOnlyGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Universal Not Crime Only Scraper")
    ...
    def _header(self):
        ...
        tk.Label(hdr, text="Universal Not-Crime Only Scraper",
                 font=(FONT_FAMILY, 15, "bold"),
                 bg=SURFACE, fg=TEXT).pack(side="left", pady=10)
        tk.Label(hdr, text="  —  Dhamaan xogaha aan crime ahayn kaliya meel ka baadh",
        ...
        while pending_urls and len(self._items) < target and not self._stop_flag:
            ...
            try:
                ...
                if len(full_text) >= 100:
                    ...
                    if cat == "not crime-related":
                        self._items.append({
                            "url": current_url, "text": title_text + "\n" + full_text,
                            "category": cat
                        })
                        self._log_msg(f"✅ [{len(self._items)}/{target}] {title_text[:50]}", "ok")
                        self._update_stats()
        ...
        while len(self._items) < target and no_new < 10 and not self._stop_flag:
            ...
            for block in blocks:
                ...
                if cat == "not crime-related":
                    ...
                    gained += 1
                    self._log_msg(f"[{len(self._items)}/{target}] {platform}: {text[:60]}...", "ok")
                    self._update_stats()
                    self._set_progress(len(self._items), target)
